...
## 720p derived-media ceiling — 2026-09-25

Every clip, frame, and crop derived from source video is now downscaled to at most 720p and never upscaled: encoded clips (the existing `--height`, now also the hard maximum), OCR detection frames, verification frames, and crops taken from those frames. Source files are never modified. The ceiling bounds provider payload size independently of source resolution — the nine-frame OCR sampling measured 13.3 MB base64 from 1080p sources and would exceed transport limits from 4K material; at the ceiling the same request is bounded regardless of input. Cache invalidations: OCR stage keys now include frame height, and verification keys moved to version 4, so artifacts and verdicts derived from full-resolution media are recomputed rather than replayed. Transcript and visual caches are unaffected. Tradeoff: small-text and plate legibility can drop relative to full-resolution frames; unreadable stays unreadable, and prompts no longer claim original resolution. Originals remain available on disk for a future higher-resolution extraction path. Regression checks cover frame downscaling/never-upscaling with real ffmpeg, the height ceiling on both configs, and OCR cache invalidation on height change.
